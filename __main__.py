...
from snapshots import Snapshot, System, get_systems
from ui import UI
...
def get_rollback_commands(
    main_subvol: Path, snapshot_subvol: Path, post_rollback_script: Path | None
) -> list[Command]:
    commands = [
        Command("btrfs", ["subvolume", "delete", main_subvol.as_posix()]),
        Command(
            "btrfs",
            [
                "subvolume",
    ...
    if post_rollback_script is not None:
        commands.append(Command("bash", [post_rollback_script.as_posix()]))
    return commands
...
def show_snapshots_page(
    ui: UI, system: System, snapshots: list[Snapshot]
) -> Snapshot | None:
...
def show_confirmation_page(
    ui: UI, system: System, snapshot: Snapshot, commands: list[Command]
) -> bool:
    index = ui.show_selection_page(
        [
            f"Operating system: {system}",
...
def show_reboot_page(ui: UI, text: list[str]) -> bool:
    index = ui.show_selection_page([*text], ["Reboot", "Exit"])
    return index == 0
...
def main(ui: UI) -> None:
    ...
    while True:
        ...
        while True:
            ...
            rollback_commands = get_rollback_commands(
                system.main_subvol, snapshot.subvol, system.post_rollback_script
            )
            confirmation = show_confirmation_page(
                ui, system, snapshot, rollback_commands
            )
            if not confirmation:
                continue
